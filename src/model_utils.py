...
def apply_dim_reduction(X_train_scaled, X_test_scaled, y_train, n_components=20):
    ...
    dim_reduction_results['PCA'] = {'X_train': X_train_pca, 'X_test': X_test_pca, 'time': pca_time}
    dr_models['PCA'] = pca
    ...
    dim_reduction_results['ICA'] = {'X_train': X_train_ica, 'X_test': X_test_ica, 'time': ica_time}
    dr_models['ICA'] = ica
    ...
    dim_reduction_results['LDA'] = {'X_train': X_train_lda, 'X_test': X_test_lda, 'time': lda_time}
    dr_models['LDA'] = lda
    ...
    dim_reduction_results['Autoencoder'] = {'X_train': X_train_ae, 'X_test': X_test_ae, 'time': ae_time}
    dr_models['Autoencoder'] = encoder_model
    print(f"Autoencoder tamamlandı. Süre: {ae_time:.2f} saniye")

    # 5. LLE
    print("\n5. LLE uygulanıyor (küçük sample ile)...")
    sample_size = 5000
    ...
    X_train_lle_sample = lle.fit_transform(X_train_sample)
    lle_time = time.time() - start_time
    print(f"LLE tamamlandı (sample üzerinde). Süre: {lle_time:.2f} saniye")
...
def get_models():
    return {
        ...
        'ANN': MLPClassifier(hidden_layer_sizes=(64, 32), max_iter=50, random_state=42)
    }